...
static void protocol_background_slice();
...
void protocol_main_loop()
{
  ...
  if (sys.state & (STATE_ALARM | STATE_SLEEP)) {
    ...
    // All systems go!
    system_execute_startup(line); // Execute startup script. 执行启动脚本。
  }
...
void protocol_auto_cycle_start()
{
  if (plan_get_current_block() != NULL) { // Check if there are any blocks in the buffer.
    system_set_exec_state_flag(EXEC_CYCLE_START); // If so, execute them!  置位EXEC_CYCLE_START
  }
}
...
//Grbl 实时命令执行系统的总接口
//此函数只在主流程里被调用，不在中断里调用
void protocol_execute_realtime()
{

  Servo_ProcessPending();	//更新PWM
  ...
  protocol_background_slice(); //每隔5ms osDelay(1)
}
...
static void protocol_background_slice()
{
  ...
  if (!(sys.state & (STATE_CYCLE | STATE_HOLD | STATE_JOG | STATE_HOMING | STATE_SAFETY_DOOR))) {
    return;
  }

  now = osKernelGetTickCount();
  if ((uint32_t)(now - last_slice_tick) < 5U) {
    return;
  }
...
void protocol_exec_rt_system()
{
  ...
  rt_exec = sys_rt_exec_alarm; // Copy volatile sys_rt_exec_alarm.  复制易变变量 sys_rt_exec_alarm
  if (rt_exec) { // Enter only if any bit flag is true 仅当任意位标志为真时进入
        ...
        if ((sys.state == STATE_IDLE) || ((sys.state & STATE_HOLD) && (sys.suspend & SUSPEND_HOLD_COMPLETE))) {
          if (sys.state == STATE_HOLD && sys.spindle_stop_ovr) {
            ...
            if (plan_get_current_block() && bit_isfalse(sys.suspend,SUSPEND_MOTION_CANCEL)) {
              ...
              st_wake_up();	//开启定时器2
            } else { // Otherwise, do nothing. Set and resume IDLE state.  否则，不执行任何操作。设置并恢复空闲状态。
              sys.suspend = SUSPEND_DISABLE; // Break suspend state.
              sys.state = STATE_IDLE;
