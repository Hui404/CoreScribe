...
- 软件层：FreeRTOS 任务调度、LVGL 图形界面移植、Grbl 指令解析、CoreXY 运动学解算
...
├── Drawings/       # 工程图纸 (*.slddrw) 可选
└── Export/         # 通用格式（无SW也可查看）
    ├── CoreScribe_CoreXY.stp  # STEP格式（可编辑）
    └── CoreScribe_CoreXY.stl  # STL格式（3D打印/预览）
...
- **无SolidWorks解决方案**：使用 `CAD/Export/` 下的STEP/STL文件，借助FreeCAD、Blender、在线3D查看器打开

- **大文件说明**：SW源文件已启用Git LFS管理，克隆仓库时自动拉取完整文件
...
1. CoreXY 机械结构组装，确认同步带张力、限位开关安装位置；

2. STM32 最小系统与电机驱动模块接线；

3. 外设连接：触摸屏（FSMC接口）、串口（USART1）、电源接口；

4. 硬件自检：通电后检查电机、限位开关、屏幕是否正常。
...
|LVGL界面|触摸屏操作「点位控制」「参数设置」，确认界面响应与显示正常|
